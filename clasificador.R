...
ggplot(xlong, aes(x= meses, y = prop, fill = variable)) + geom_bar(stat = 'identity') + xlab("Meses sin actividad") + ylab("Porcentaje de Activos/No Activos") + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="bold"),legend.title = element_text(size=18,face="bold"),legend.text = element_text(size=20))
graphics.off()
